 ...
 */
export function isQualityAcceptable(result: DataQualityResult): boolean {
 ...
 */
export function processData(data: StockDataPoint[]): DataQualityResult {
	if (data.length === 0) {
		return {
			data: [],
			gapsDetected: 0,
 ...
 */
function countMissingDays(data: StockDataPoint[]): number {
	if (data.length < 2) {
		return 0;
	}
 ...
 */
function detectGaps(data: StockDataPoint[]): {endIndex: number; gapDays: number; startIndex: number}[] {
...
function interpolateGaps(
	...
	for (const gap of gaps.toReversed()) {
		...
		if (gap.gapDays <= MAX_INTERPOLATION_GAP + 1) {
			...
			const interpolatedPoints = linearInterpolate(startPoint, endPoint, gap.gapDays - 1);
 ...
 */
function linearInterpolate(start: StockDataPoint, end: StockDataPoint, numPoints: number): StockDataPoint[] {
